...
import { PatnaLocation } from '@/data/patna-locations'

interface PataLocationPageProps {
  loc: PatnaLocation
}

export function generatePatnaLocationMetadata(loc: PatnaLocation): Metadata {
  const title = `${loc.name} Escorts in Patna | Call Girls in ${loc.name} | Hot Sanjana`
...
export default function PatnaLocationPage({ loc }: PataLocationPageProps) {
  const faqItems = [
    {
      q: `What is ${loc.name} known for in Patna?`,
      a: `${loc.name} is an important area of Patna with a unique character shaped by its location and community. The area features ${loc.landmarks.slice(0, 2).join(', ')} and serves as a key neighbourhood in Patna.`,
    },
    {
    ...
    },
    {
      ...
      a: `Our escorts in ${loc.name} are fully mobile and can visit nearby neighbourhoods including ${loc.nearbyAreas.slice(0, 3).join(', ')}. We serve the entire greater Patna area.`,
    },
    {
    ...
    },
    {
      ...
      a: `Yes, our escorts are experienced in providing professional companionship for events, dinners, and occasions in ${loc.name} and throughout Patna.`,
    },
    {
    ...
    },
    {
      ...
      a: `Yes, our escorts in ${loc.name} provide both incall and outcall services throughout Patna with full professional discretion.`,
    },
    {
      ...
      <div className="bg-gradient-to-r from-red-600 to-red-700 text-white py-16">
        <div className="max-w-6xl mx-auto px-4">
          <div className="inline-block bg-white/20 px-4 py-2 rounded-full mb-4">
            <span className="text-sm font-semibold">{loc.area} · Patna</span>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
              <h2 className="text-3xl font-bold mb-6 text-gray-900">
                Premium Escorts in {loc.name}, Patna
              </h2>
              <p className="text-gray-700 leading-relaxed mb-4">
                {loc.uniqueContent}
              </p>
              <p className="text-gray-600 leading-relaxed">
                Our verified escorts in {loc.name} provide the highest standards of professional companionship and discretion. Each escort is thoroughly vetted and available 24/7 for outcalls and incalls throughout {loc.name} and greater Patna.
              </p>
            </div>
          ...
          <div>
            ...
            {/* Other Patna Locations */}
            <div className="bg-white rounded-lg shadow-lg p-6">
              <h4 className="text-lg font-bold mb-4 text-gray-900">More Patna Locations</h4>
              <div className="space-y-2">
                <Link href="/escorts/patna-junction" className="block text-red-600 hover:text-red-700 font-medium">
                  Patna Junction
                </Link>
                <Link href="/escorts/boring-road" className="block text-red-600 hover:text-red-700 font-medium">
                  Boring Road
                </Link>
                <Link href="/escorts/rajendra-path" className="block text-red-600 hover:text-red-700 font-medium">
                  Rajendra Path
                </Link>
                <Link href="/escorts/patna-airport" className="block text-red-600 hover:text-red-700 font-medium">
                  Patna Airport
                </Link>
                <Link href="/escorts/ashok-rajpath" className="block text-red-600 hover:text-red-700 font-medium">
                  Ashok Rajpath
                </Link>
              </div>
            </div>
