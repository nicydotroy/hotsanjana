...
import { getAllLucknowLocationSlugs } from '@/data/lucknow-locations'
import { getAllGoaLocationSlugs } from '@/data/goa-locations'
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const lucknowLocationSlugs = getAllLucknowLocationSlugs()
  const goaLocationSlugs = getAllGoaLocationSlugs()
  ...
  const lucknowEscortPages: MetadataRoute.Sitemap = lucknowLocationSlugs.map((slug) => ({
    url: `${BASE_URL}/escorts/${slug}`,
    lastModified: new Date(),
    changeFrequency: 'weekly',
    priority: 0.85,
  ...
  const goaEscortPages: MetadataRoute.Sitemap = goaLocationSlugs.map((slug) => ({
    url: `${BASE_URL}/escorts/${slug}`,
    lastModified: new Date(),
    changeFrequency: 'weekly',
    priority: 0.85,
  ...
  return [...staticPages, ...escortPages, ...patnaEscortPages, ...lucknowEscortPages, ...goaEscortPages, ...callGirlsPages, ...typePages]
}
